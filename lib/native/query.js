...
var prepareValue = function(val, stringifyNull) {
  ...
  if (Array.isArray(val)) {
    ...
    return "{" + val.map(function(v) { return prepareValue(v, true); }).join(",") + "}";
  }
  if(val instanceof Date || val instanceof Object) {
    return JSON.stringify(val);
  }
  return val === null ? (stringifyNull ? "NULL" : null) : val.toString();
};
...
var NativeQuery = function(text, values, callback) {
  ...
  //normalize values
  if(this.values) {
    this.values = this.values.map(function(o,i,a) { return prepareValue(o, false); });
  }
